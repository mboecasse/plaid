...
var APP_SECRET_SESSION = envvar.string('APP_SECRET_SESSION', 'dsfljdsjklfnsqfneilzuflez');
...
app.get('/confirm-payment.html', async function(request, response, next) {
  try {
    ...
    const uri = `https://creator.zoho.com/api/mboecasse1/xml/pardna-v3/form/Payment_Initiation/record/update?status=${transactionStatus}&criteria=reference_id%3D%22${referenceId}%22&authtoken=${ZOHO_ACCESS_TOKEN}`;
    console('POST : ', uri);
...
// payment token so that the developer is not required to supply one.
// This makes the quickstart easier to use.
app.post('/init_payment', function(request, response, next) {
